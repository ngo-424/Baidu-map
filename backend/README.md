...
三类 POI 的独立内部服务、固定 4×4 查询计划、分页／分类／UID 去重、审计账本和回放入口见[任务 B1 接口与验收说明](docs/任务B_POI接口与验收说明.md)。在线入口默认拒绝联网；先用合成夹具运行 `python -m tools.poi_collect --mode plan` 和 `--mode replay`，真实采集需另行批准额度、QPS、AK 权限和运行窗口。
