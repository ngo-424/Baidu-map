...
async function mockMap(page: Page) {
  await page.addInitScript(() => {
    ...
    Object.assign(window, { BMapGL: { Map, Point, Polygon, Marker: Overlay, Label, Polyline: Overlay } });
  });
}
...
test('facility report, category filtering, route and time layers share one analysis', async ({page},info)=>{
  ...
  await page.route('**/api/analyses/*/result',async route=>{
    ...
    data.facilityAnalysis={status:'partial',queries:[{category:'pharmacy',query:'药店',status:'truncated',pages:2,returned:1,excluded:0,invalid:0,total:150,reason:'page_limit'}],assessments:[{location,duration_s:500,categories:[{category:'shopping',status:'unknown',facility_id:null,distance_m:null,reason:'incomplete'},{category:'medical',status:'covered',facility_id:'pharmacy-fixture',distance_m:600,reason:'walking'},{category:'education',status:'unknown',facility_id:null,distance_m:null,reason:'incomplete'}]}],candidate_points:2,assessed_points:1,unassessed_points:1,network_requests:0,elapsed_seconds:0,search_radius_m:3500,routes:{'pharmacy-fixture':savedRoute},serviceBlindRegions:{shopping:{type:'MultiPolygon',coordinateSystem:'bd09ll',coordinates:[]},medical:{type:'MultiPolygon',coordinateSystem:'bd09ll',coordinates:[]},education:{type:'MultiPolygon',coordinateSystem:'bd09ll',coordinates:[]}},warnings:['离线样例，未测点不计入盲区。']};
    await route.fulfill({response,json:data});
  });
  await page.route('**/api/analyses/*/routes/*',r=>r.fulfill({json:savedRoute}));
  await page.goto('/');
  await analyze(page);
